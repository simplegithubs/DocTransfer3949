  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#1e293b' }}>
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '4rem 1.5rem' }}>
        ...
        {/* Explore More Alternatives Section */}
        <section style={{ marginTop: '5rem', borderTop: '1px solid #e2e8f0', paddingTop: '4rem' }}>
          <h2 style={{ fontSize: '1.75rem', fontWeight: '800', color: '#0f172a', marginBottom: '1.5rem', textAlign: 'center' }}>
            Explore Alternatives by Niche & Industry
          </h2>
          <p style={{ textAlign: 'center', color: '#64748b', marginBottom: '2.5rem', maxWidth: '600px', margin: '0 auto 2.5rem auto', lineHeight: 1.5 }}>
            Find secure, E2EE document sharing and legally valid e-signature tools tailored exactly to your industry workflow.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', justifyContent: 'center' }}>
            {allSEOPages.filter(p => p.category === 'alternatives').map((page) => (
              <Link
                key={page.slug}
                to={`/alternatives/${page.slug}`}
                style={{
                  background: 'white',
                  border: '1px solid #e2e8f0',
              ...
              >
                {page.title.replace('Best Free ', '').replace(' (2026)', '').replace(' Alternatives', '').replace(' Alternative', '')}
              </Link>
            ))}
          </div>
        </section>
